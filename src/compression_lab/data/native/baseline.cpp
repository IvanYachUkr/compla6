// Compression Lab 0.3.0 conventional native control. SPDX-License-Identifier: MIT
#include <algorithm>
#include <array>
#include <cstdint>
#include <cstdio>
...
#endif
static_assert(CL_TRANSFORM >= 0 && CL_TRANSFORM <= 2, "unsupported transform");
constexpr const char* ARCHIVE_TAG = CL_TRANSFORM == 0 ? "CLB1" : "CLT1";
#ifndef CL_CODEC
#define CL_CODEC 0
#endif
#ifndef CL_LEVEL
...
uint32_t update(uint32_t c,const uint8_t*p,size_t n){return lab::crc_update(c,p,n);}
uint32_t crc(const B&b){return lab::crc32(b.data(),b.size());}
...
B encode(const B&b,const B&dict){B p;if(!b.empty()){if constexpr(CL_TRANSFORM==0)p=compress(b,dict);else p=compress(lab::forward(b,CL_TRANSFORM),dict);}uint8_t codec=CL_CODEC;if(p.size()>=b.size()){p=b;codec=0;}B o(ARCHIVE_TAG,ARCHIVE_TAG+4);o.insert(o.end(),{1,codec,uint8_t(codec?CL_TRANSFORM:0),0});put64(o,b.size());put64(o,p.size());put32(o,crc(b));put32(o,0);o.insert(o.end(),p.begin(),p.end());uint32_t c=~update(update(0xffffffffU,o.data(),28),o.data()+32,o.size()-32);for(int i=0;i<4;i++)o[28+i]=uint8_t(c>>(24-8*i));return o;}
B decode(const B&b,const B&dict){need(b.size()>=32&&!memcmp(b.data(),ARCHIVE_TAG,4),"bad archive");need(b[4]==1&&b[6]==uint8_t(b[5]?CL_TRANSFORM:0)&&!b[7]&&(b[5]==0||b[5]==CL_CODEC),"bad archive flags");Reader r{b};r.p=8;auto raw=r.u64(),size=r.u64();auto c=r.u32(),outer=r.u32();need(raw<=LIMIT&&size==b.size()-32,"bad archive lengths");need(outer==~update(update(0xffffffffU,b.data(),28),b.data()+32,b.size()-32),"archive checksum");B o;if(b[5]==0){need(raw==size,"stored length");o.assign(b.begin()+32,b.end());}else{o.resize(raw);
#if CL_CODEC == 1
...
#endif
 }if(b[5]&&CL_TRANSFORM)o=lab::inverse(o,CL_TRANSFORM);need(o.size()==raw&&crc(o)==c,"raw checksum");return o;}
