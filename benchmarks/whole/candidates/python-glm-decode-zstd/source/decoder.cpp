// pypack-bal decoder: zstd frame decode + indentation-transform inversion.
// Independent reconstruction from the archive plus pinned libzstd only.
#include "format.h"
#include <zstd.h>
#include <cstring>
#include <stdexcept>
...
std::vector<uint8_t> decompress_bal(const std::vector<uint8_t>& arc) {
  uint64_t orig_len = 0, want = 0;
  size_t body = 0;
  uint8_t flags = 0;
  if (!get_header(arc.data(), arc.size(), 3, orig_len, body, flags, want) ||
  ...
  size_t ds = ZSTD_decompressDCtx(d, tr.data(), tr.size(), arc.data() + body,
                                  arc.size() - body);
  ZSTD_freeDCtx(d);
  if (ZSTD_isError(ds) || ds > tr.size())
    throw std::runtime_error("zstd decode failed");
  tr.resize(ds);
...
int decode_stream_main() {
  ...
  for (size_t idx = 0; idx < recs.size(); idx++) {
    ...
    r.alias = recs[idx].alias;
    try {
      r.payload = decompress_bal(recs[idx].payload);
    } catch (const std::exception& e) {
      fprintf(stderr, "pypack: corrupt archive, object %zu (%s)\n", idx, e.what());
      return 2;
    }
