...
template <class F> void parallel(size_t n, F fn) {
  unsigned t = unsigned(std::min<size_t>(workers(), n));
  if (t <= 1) {
...
#endif
static_assert(BLOCK_BITS == 27, "global stripes require 128 MiB whole-object blocks");
constexpr size_t BLOCK = size_t(1) << BLOCK_BITS;
...
inline Bytes stripesdecode(const uint8_t *p, size_t n, size_t raw) {
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    uint8_t mode = r.byte();
    require(size > 0 && size <= raw - total && mode <= 2, "bad stripe dimensions");
    require(mode == 0 ? payload == size : payload < size, "bad stripe payload length");
    stripes.push_back({r.take(payload), size, payload, total, mode});
    total += size;
  }
  ...
  parallel(stripes.size(), [&](size_t i) {
    ...
    if (s.mode == 0) std::memcpy(out.data() + s.at, s.p, s.raw);
    else if (s.mode == 1) lzdecode(s.p, s.n, out.data() + s.at, s.raw);
    else {
      Bytes decoded = entropy_decode(s.p, s.n, s.raw);
      require(decoded.size() == s.raw, "wrong entropy stripe output size");
      std::memcpy(out.data() + s.at, decoded.data(), s.raw);
    }
  });
  return out;
}
inline Bytes decode(const Bytes &in) {
  ...
  require(total <= BLOCK, "output too large");
  uint32_t count = r.u32();
  require(count == (total != 0), "block count mismatch");
  if (!count) {
    r.end();
    return {};
  }
  uint8_t mode = r.byte();
  uint32_t raw = r.u32(), prep_size = r.u32(), n = r.u32();
  uint64_t hash = r.u64();
  require(mode <= 1, "unknown block mode");
  require(raw == total, "block length mismatch");
  require(prep_size <= raw + 5 && n <= prep_size + 32, "block payload too large");
  if (mode == 0)
    require(n == prep_size, "raw length mismatch");
  const uint8_t *p = r.take(n);
  r.end();
  Bytes prep;
  if (mode)
    prep = stripesdecode(p, n, prep_size);
  else if (prep_size)
    prep.assign(p, p + prep_size);
  Bytes out = columnar::inverse(prep, raw);
  require(checksum(out.data(), out.size()) == hash, "checksum mismatch");
  return out;
}
} // namespace sw
