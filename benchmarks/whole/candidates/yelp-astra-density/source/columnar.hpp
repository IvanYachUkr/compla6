...
inline void put32(Bytes& out, uint32_t x) {
    for (unsigned i = 0; i < 4; ++i) out.push_back(uint8_t(x >> (i * 8)));
}
inline void put64(Bytes& out, uint64_t x) {
    for (unsigned i = 0; i < 8; ++i) out.push_back(uint8_t(x >> (i * 8)));
}
struct Reader {
    const uint8_t* p;
    size_t left;
    const uint8_t* take(size_t n) {
        if (n > left) throw std::runtime_error("truncated column transform");
    ...
    uint64_t u64() {
        ...
        uint64_t x = 0;
        for (unsigned i = 0; i < 8; ++i) x |= uint64_t(q[i]) << (i * 8);
        return x;
    }
};
...
inline void varput(Bytes& out, uint32_t x) {
    while (x >= 128) { out.push_back(uint8_t(x) | 128); x >>= 7; }
...
inline uint32_t varget(Reader& r) {
    ...
    for (unsigned shift = 0; shift <= 28; shift += 7) {
        uint8_t c = *r.take(1);
        if (shift == 28 && c >= 16) throw std::runtime_error("dictionary varint overflow");
        x |= uint32_t(c & 127) << shift;
        if (!(c & 128)) {
            if (shift && c == 0) throw std::runtime_error("noncanonical dictionary varint");
            return x;
        }
    }
...
struct SliceHash {
    size_t operator()(std::string_view s) const {
        ...
        while (n--) h = (h ^ uint8_t(*p++)) * 0x100000001b3ULL;
...
inline Bytes pack_pairs(const Bytes& input, uint32_t rows) {
    ...
    for (uint32_t row = 0; row < rows; ++row) {
        const uint8_t* stop = static_cast<const uint8_t*>(std::memchr(p, '\n', size_t(end - p)));
        if (!stop) throw std::runtime_error("internal missing column delimiter");
        const uint8_t* q = p;
        bool object = q != stop && *q++ == '{';
        row_ids.clear();
        if (object) {
            if (q == stop) object = false;
            while (object && *q != '}') {
                const uint8_t* member = q;
                if (*q != '"' || !skip_string(q, stop) ||
                    q == stop || *q++ != ':' || !skip_value(q, stop) || q == stop) {
                    object = false; break;
                }
                std::string_view text(reinterpret_cast<const char*>(member), size_t(q - member));
                auto found = dictionary.find(text);
                uint32_t id;
                if (found == dictionary.end()) {
                    if (entries.size() >= 1u << 20) return {};
                ...
                if (*q == '}') break;
                if (*q++ != ',' || q == stop || *q != '"') object = false;
            }
            if (object && ++q != stop) object = false;
        }
        if (object) {
            varput(codes, uint32_t(row_ids.size()) + 1);
            for (uint32_t id : row_ids) varput(codes, id);
        } else {
            varput(codes, 0);
        ...
        p = stop + 1;
        if (dictionary_bytes + codes.size() >= input.size()) return {};
    }
    if (p != end) throw std::runtime_error("internal extra column rows");
    Bytes out;
...
inline bool text_candidate(const Bytes& input, uint32_t rows) {
    ...
    if constexpr (!List) unique.reserve(sample);
    unsigned lists = 0;
    for (uint32_t i = 0; i < sample; ++i) {
        ...
        if constexpr (List) {
            if (s.size() >= 2 && s.front() == '"' && s.back() == '"' &&
                s.find(", ") != std::string_view::npos) ++lists;
        } else unique.insert(s);
        p = stop + 1;
    }
    if constexpr (List) return lists * 4 >= sample;
    else return unique.size() * 4 <= sample * 3;
}
...
template<bool List>
inline Bytes pack_text(const Bytes& input, uint32_t rows) {
    if (!text_candidate<List>(input, rows)) return {};
    const uint8_t* p = input.data();
    const uint8_t* end = p + input.size();
    std::unordered_map<std::string_view, uint32_t, SliceHash> dictionary;
    dictionary.reserve(4096);
    ...
    for (uint32_t row = 0; row < rows; ++row) {
        ...
        if constexpr (List) {
            if (s.size() >= 2 && s.front() == '"' && s.back() == '"') {
                ...
                for (;;) {
                    ...
                    size_t length = next == std::string_view::npos ? s.size() - start : next - start;
        ...
        } else varput(codes, intern(s));
        if (entries.size() > (1u << 20) || dictionary_bytes + codes.size() >= input.size()) return {};
        p = stop + 1;
    }
    if (p != end) throw std::runtime_error("internal extra column rows");
    Bytes out;
...
inline Bytes unpack_dictionary(const uint8_t* src, size_t n, size_t raw, uint32_t rows, uint8_t mode) {
    Reader r{src, n};
    ...
    auto append = [&](const uint8_t* p, size_t size) {
        if (size > raw - out.size()) throw std::runtime_error("dictionary output overflow");
        out.insert(out.end(), p, p + size);
    };
    auto byte = [&](uint8_t c) {
        if (out.size() == raw) throw std::runtime_error("dictionary output overflow");
        out.push_back(c);
    };
    for (uint32_t row = 0; row < rows; ++row) {
        uint32_t code = varget(r);
        if (mode == 3) {
            if (code >= dictionary.size()) throw std::runtime_error("bad scalar dictionary ID");
            std::string_view s = dictionary[code];
            append(reinterpret_cast<const uint8_t*>(s.data()), s.size());
        } else if (code == 0) {
            uint32_t size = varget(r);
            const uint8_t* text = r.take(size);
            if (std::memchr(text, '\n', size)) throw std::runtime_error("bad raw dictionary row");
            append(text, size);
        } else {
            ...
            for (uint32_t i = 0; i < members; ++i) {
                ...
                if (id >= dictionary.size()) throw std::runtime_error("bad dictionary ID");
                if (i) { byte(','); if (mode == 2) byte(' '); }
    ...
    if (r.left || out.size() != raw) throw std::runtime_error("wrong dictionary output size");
    return out;
}
...
// in the block are retained verbatim. The enclosing codec chooses line-aligned block sizes.
inline Bytes forward_block(const uint8_t* src, size_t n) {
    if (n == 0 || n > (uint64_t(1) << 30)) return raw_block(src, n);
    std::vector<Bytes> schemas, columns;
    uint32_t rows = 0;
    const uint8_t* p = src;
    const uint8_t* end = src + n;
    while (p != end) {
        const uint8_t* line_end = static_cast<const uint8_t*>(
            std::memchr(p, '\n', size_t(end - p)));
        if (!line_end) line_end = end;
        if (p == line_end || *p++ != '{') return raw_block(src, n);
        size_t col = 0;
        if (p == line_end) return raw_block(src, n);
        while (*p != '}') {
            const uint8_t* key = p;
            if (*p != '"' || !skip_string(p, line_end) ||
                p == line_end || *p++ != ':') return raw_block(src, n);
            size_t key_len = size_t(p - key);
            if (rows == 0) {
                if (col == 1024) return raw_block(src, n);
                schemas.emplace_back(key, p);
                columns.emplace_back();
            } else if (col >= schemas.size() ||
                       schemas[col].size() != key_len ||
                       std::memcmp(schemas[col].data(), key, key_len) != 0) {
                return raw_block(src, n);
            }
            const uint8_t* value = p;
            if (!skip_value(p, line_end) || p == line_end) return raw_block(src, n);
            columns[col].insert(columns[col].end(), value, p);
            columns[col].push_back('\n');
            ++col;
            if (*p == '}') break;
            if (*p++ != ',' || p == line_end || *p != '"') return raw_block(src, n);
        }
        if (++p != line_end || col != schemas.size() ||
            rows == std::numeric_limits<uint32_t>::max()) return raw_block(src, n);
        ++rows;
    ...
    for (size_t i = 0; i < columns.size(); ++i)
        transformed_size += 21 + schemas[i].size() + (packed[i].empty() ? columns[i].size() : packed[i].size());
    if (transformed_size >= n + 5) return raw_block(src, n);
    Bytes out;
    out.reserve(transformed_size);
    out.insert(out.end(), {'J', 'C', 'L', '1', 2, uint8_t(src[n - 1] == '\n'), 0, 0});
    put64(out, n);
    put32(out, rows);
...
inline Bytes inverse_block(const uint8_t* src, size_t n,
                           size_t expected_raw = std::numeric_limits<size_t>::max()) {
    Reader reader{src, n};
    const uint8_t* head = reader.take(5);
    if (std::memcmp(head, "JCL1", 4) != 0) throw std::runtime_error("bad column magic");
    if (head[4] == 0) {
        if (expected_raw != std::numeric_limits<size_t>::max() && reader.left != expected_raw)
            throw std::runtime_error("wrong raw column output size");
        return Bytes(reader.p, reader.p + reader.left);
    }
    if (head[4] != 1 && head[4] != 2) throw std::runtime_error("bad column mode");
    const uint8_t* flags = reader.take(3);
    if (flags[0] > 1 || flags[1] || flags[2]) throw std::runtime_error("bad column flags");
    ...
    if ((expected_raw != std::numeric_limits<size_t>::max() && raw_size != expected_raw) ||
    ...
    uint64_t expected = uint64_t(rows) * (2 + (count ? count - 1 : 0)) + rows - !flags[0];
    for (uint32_t i = 0; i < count; ++i) {
        ...
        if (schema_size > raw_size || data_size < rows || data_size > raw_size ||
            packed_size > reader.left || mode > 3 || (mode == 0 && packed_size != data_size))
            throw std::runtime_error("bad column lengths");
        expected += uint64_t(rows) * schema_size + data_size - rows;
        if (expected > raw_size) throw std::runtime_error("inconsistent column lengths");
        columns.push_back({schema, schema_size, size_t(data_size), size_t(packed_size), mode, nullptr, nullptr});
    }
    if (expected != raw_size) throw std::runtime_error("inconsistent column output size");
    std::vector<Bytes> unpacked(count);
    for (size_t i = 0; i < columns.size(); ++i) {
        Column& c = columns[i];
        c.p = reader.take(c.packed_size);
        if (c.mode) {
            unpacked[i] = unpack_dictionary(c.p, c.packed_size, c.data_size, rows, c.mode);
            c.p = unpacked[i].data();
        }
        c.end = c.p + c.data_size;
    }
    if (reader.left) throw std::runtime_error("trailing column bytes");
    Bytes out(size_t(raw_size), uint8_t(0));
    uint8_t* dest = out.data();
    uint8_t* dest_end = dest + out.size();
    for (uint32_t row = 0; row < rows; ++row) {
        if (dest == dest_end) throw std::runtime_error("column output overflow");
        *dest++ = '{';
        for (size_t i = 0; i < columns.size(); ++i) {
            Column& c = columns[i];
            const uint8_t* stop = static_cast<const uint8_t*>(
                std::memchr(c.p, '\n', size_t(c.end - c.p)));
            if (!stop) throw std::runtime_error("missing column delimiter");
        ...
        if (suffix > size_t(dest_end - dest)) throw std::runtime_error("column output overflow");
        *dest++ = '}';
        if (row + 1 < rows || flags[0]) *dest++ = '\n';
    }
    for (const Column& c : columns)
        if (c.p != c.end) throw std::runtime_error("extra column values");
    if (dest != out.data() + out.size()) throw std::runtime_error("wrong column output size");
    return out;
}

inline Bytes forward(const Bytes& input) { return forward_block(input.data(), input.size()); }
