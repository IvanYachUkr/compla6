...
constexpr uint64_t max_object_bytes = uint64_t{512} << 20;
constexpr uint64_t max_batch_bytes = uint64_t{1} << 30;
...
inline void charge(uint64_t &used, uint64_t amount) {
  if (amount > max_batch_bytes - used)
    fail("batch exceeds 1 GiB limit");
  used += amount;
}
...
inline void check_object_size(uint64_t size) {
  if (size > max_object_bytes)
    fail("object exceeds 512 MiB limit");
}
